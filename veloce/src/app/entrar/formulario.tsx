...
export function FormularioEntrar({ siguiente }: { siguiente?: string }) {
  ...
  return (
    <form action={accion} className="mt-8 space-y-4">
      ...
      {estado.error && (
        <p role="alert" className="border-l-2 border-marca bg-marca-suave px-4 py-3 text-sm text-marca-oscuro">
          {estado.error}
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={pendiente}
        className="w-full bg-tinta px-6 py-3.5 text-sm font-bold text-papel transition-colors hover:bg-marca disabled:opacity-60"
      >

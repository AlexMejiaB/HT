...
const estiloBase =
  "inline-flex items-center justify-center gap-2.5 px-9 py-4 text-[0.7rem] font-normal uppercase tracking-[0.22em] transition-all duration-300";
...
export function BotonWhatsapp({
  ...
  return (
    <a
      ...
      className={cn(
        estiloBase,
        variante === "solido" && "bg-marca text-tinta hover:bg-marca-claro",
        variante === "contorno" &&
          "border border-marca/50 text-marca hover:border-marca hover:bg-marca hover:text-tinta",
        variante === "claro" &&
    ...
    >
      ...
      >
        <button
          ...
          onClick={alCerrar}
          aria-label="Cerrar"
          className="absolute -top-11 right-0 inline-flex items-center gap-2 text-[0.7rem] uppercase tracking-[0.22em] text-papel hover:text-marca"
        >
          Cerrar <X className="size-3.5" aria-hidden="true" />
        </button>
        <iframe
